...
verify_phone() {
    ...
    re1="^[0-9]+$"                          # check if consists of number
    if  ! [[ "$phone" =~ $re1 ]]
    then
...
add_new_contact(){
    ...
    verify_phone
    verify_email
    # write the new contact data to a new line in the file
    save_contact
}
...
update(){
    # update specific field and save it
...
# Request the file name from user and verify its existance
check_file

# launch the program after reading the file (show the menu)
menu
